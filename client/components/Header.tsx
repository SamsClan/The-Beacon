...
export default function Header() {
  ...
  const botImage =
    "https://th.bing.com/th/id/OIP.4FuBjYmRvtEXBS-bHi63RQAAAA?w=204&h=186&c=7&r=0&o=7&dpr=1.3&pid=1.7";
  const meImage =
    ...
    if (!botInitialized && window.botpress?.init) {
      ...
      window.botpress.on("webchat:ready", () => {
        ...
        window.botpress.on("incoming_message", (event: any) => {
          ...
          if (userInteracted && window.responsiveVoice && text) {
        ...
        <nav className="container mx-auto px-4 flex items-center justify-between transition-all duration-300">
          ...
          {!compact && (
            <div className="hidden sm:flex flex-row font-lato gap-6">
              {["Check It", "About Us", "Article", "Help"].map((link) => (
                <a
                  key={link}
                  className="text-2xl font-semibold hover:text-beacon-yellow transition"
                  href="#"
                >
                  {link}
                </a>
          ...
          <div className="flex items-center gap-4">
            ...
            {!compact && !user && (
              <a
                href="/signup"
                className="bg-red-600 text-xl text-white px-6 py-3 rounded-lg font-lato hover:bg-red-700 transition"
              >
                Sign Up
              </a>
            )}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex">
          <div className="w-72 bg-white h-full shadow-xl p-6 animate-slideIn flex flex-col gap-6">
            ...
            {["Check It", "About Us", "Article", "Help"].map((link) => (
              <a
                key={link}
                className="text-xl font-semibold hover:text-beacon-yellow transition"
                href="#"
              >
                {link}
              </a>
            ...
            {!user && (
              <a
                href="/signup"
                className="mt-4 bg-red-600 text-white py-2 text-center rounded-lg hover:bg-red-700 transition"
              >
                Sign Up
              </a>
            )}
